# MeshCore MQTT Live Map v2.5.25
...
### v2.5.25 Feature Gallery

Version 2.5.25 keeps the Canada deployment intact while continuing the
...
controls, while collapsing repeated decoded rows across the full 24h window from
multi-observer/multi-packet reports without exposing packet hashes. Setup is still available from the Guide overlay
...
Docker Compose release metadata fallback fixes, and public JSON/WebSocket
...
docker run --rm -p 8080:8080 \
  ...
  ghcr.io/n30nex/mc-cartolive:2.5.25
```
...
The recommended v2.5.25 release path is clone + Docker Compose on a VPS or local
