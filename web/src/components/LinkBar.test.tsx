...
describe('LinkBar', () => {
  ...
  it('keeps the compact changelog focused on the current release train', () => {
    expect(LATEST_RELEASE_HIGHLIGHTS.map((item) => item.label)).toEqual(['3.2.1', '3.2.0', '3.0.1', '3.0.0']);
    expect(LATEST_RELEASE_HIGHLIGHTS.map((item) => item.title)).toContain('Continuous Live Flow');
    expect(LATEST_RELEASE_HIGHLIGHTS.map((item) => item.title)).toContain('Compact Live Platform');
