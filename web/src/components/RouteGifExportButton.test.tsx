...
describe('RouteGifExportButton', () => {
  it('renders a glowing export action for selected packet routes', () => {
    const html = renderToStaticMarkup(<RouteGifExportButton packet={packet} status="idle" progress={0} onExport={() => undefined} />);
    expect(html).toContain('Export as GIF');
    expect(html).toContain('Sender -&gt; Destination');
  });

  it('shows rendering progress while disabled', () => {
    const html = renderToStaticMarkup(<RouteGifExportButton packet={packet} status="rendering" progress={0.42} onExport={() => undefined} />);
