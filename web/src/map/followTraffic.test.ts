...
describe('follow traffic decisions', () => {
  it('uses slower camera timings for watchable live following', () => {
    expect(FOLLOW_TRAFFIC_DURATION_MS).toBeGreaterThanOrEqual(18_000);
    expect(FOLLOW_TRAFFIC_IMMEDIATE_DURATION_MS).toBeGreaterThanOrEqual(9_000);
    expect(FOLLOW_TRAFFIC_MIN_INTERVAL_MS).toBeGreaterThanOrEqual(FOLLOW_TRAFFIC_DURATION_MS + 17_000);
    expect(FOLLOW_TRAFFIC_ROUTE_MAX_ZOOM).toBeLessThanOrEqual(5.9);
    expect(FOLLOW_TRAFFIC_POINT_ZOOM).toBeLessThanOrEqual(5.3);
  });
